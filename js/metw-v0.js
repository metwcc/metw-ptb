    ...
    constructor(token) {
        ...
        if (this.token) {
            ...
            if (ok) {
                ...
                this.ws = new WebSocket(url.ws + `?id=${this.id}`)
                this.ws.onmessage = ({ data }) => {
                    var message = JSON.parse(data)
                    for (let key of Object.keys(message)) if (this['on' + key]) this['on' + key](message[key])
                }
                this.logged = true
            }
            else this.logged = false
        }
        this.indexed = { users: [], posts: [], comments: [] }
    }
...
class Comment {
    ...
    async reply(content) {
        var [response, ok] = await asyncRequest({ path: 'comments', auth: this._session.token, method: 'post', json: { content: content, parent_id: this.id, post_id: this.post_id, type: 2 } })
        return ok ? response : ok
    }
    ...
    async getReplies(offset) {
        ...
        for (let _reply = 0; _reply < replies.length; _reply++) {
            ...
            replies[_reply] = new Comment({ ...reply, reply_count: parseInt(reply.reply_count), session: this._session, user: user, post_id: this.post_id })
        }
        for (let depth = 1; depth < 3; depth++)
...
class Post {
    constructor(data) {
        ...
        this.like_count = parseInt(data.like_count)
    ...
    async like() {
        ...
        var [like, ok] = (await asyncRequest({ path: `posts/${this.id}/like`, auth: this._session.token }))
        return ok && like ? (() => { this.like_count++; this.liked = true; return true })() : false
    }
    async unlike() {
        if (!this.liked) return false
        var [like, ok] = (await asyncRequest({ path: `posts/${this.id}/unlike`, auth: this._session.token }))
        return ok && like ? (() => { this.like_count--; this.liked = false; return true })() : false
    }
    ...
    async getComments(offset, count) {
        ...
        for (let _comment = 0; _comment < comments.length; _comment++) {
            let comment = comments[_comment], user = await this._session.getUser(comment.user_id, true)
            comments[_comment] = new Comment({ ...comment, reply_count: parseInt(comment.reply_count), session: this._session, user: user, post_id: this.id })
        }
        for (let depth = 1; depth < 3; depth++)
            comments.forEach((comment) => {
                if (comment.depth == depth) comments.find((parent_comment) => comment.parent_id == parent_comment.id).replies.push(comment)
